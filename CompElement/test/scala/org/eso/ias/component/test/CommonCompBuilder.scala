...
class CommonCompBuilder(
  ...
  // The ID of the DAS where the components runs
    val sId = new Identifier(Some[String]("SUP-ID"),Some(IdentifierType.SUPERVISOR),None)

  // The ID of the DASU where the components runs
  val dasId = new Identifier(Some[String](dasuId), Some[IdentifierType](IdentifierType.DASU),Some(sId))
  ...
  for (hioType <- inputTypes) {
    val id = new Identifier(Some[String]("INPUT-HIO-ID#"+i), Some(IdentifierType.IASIO),Some(compID))
    i = i + 1
