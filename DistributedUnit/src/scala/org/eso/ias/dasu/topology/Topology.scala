...
class Topology(
   ...
   * and so on.
   */
  val levels: List[Set[String]] = buildLevels()
  ...
  def buildLevels(): List[Set[String]] = {
    ...
    /** Build the set of identifers of ASCEs that require only the 
     *  inputs in the passed set.
     * 
     * return a tuple with the identifiers of the available inputs
     *        and the new available inputs
     */
    def buildLevel(availableInputs: Set[String], allocatedAsces: Set[String]): (Set[String], Set[String]) = {
      ...
      val newAvailableInputs = availableInputs++s.map(asce => asce.output)
      (asceIds, newAvailableInputs)
    }
