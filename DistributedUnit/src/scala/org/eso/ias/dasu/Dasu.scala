...
class Dasu(
  ...
  /** When is next update scheduled */
  val nextUpdateScheduledTime = new AtomicLong(0)
  ...
  override def inputsReceived(iasios: Set[IASValue[_]]) {
    ...
    logger.debug("DASU [{}] processed the inputs {} in {} msec",id,iasios.map(x => x.id).mkString(", "),executionTime.toString())
    newOutput.foreach( output => logger.debug("DASU [{}] produced the output {}",id,output.toString()))
