...
class DocGenerator(object):
    ...
    def checkFolders(self,src,dst):
        ...
        if not os.path.exists(src):
            logger.error("The source folder %s does not exist", src)
            raise OSError("The source folder", src,"does not exist")
        elif not os.path.isdir(src):
            logger.error("The source folder %s is not a directory", src)
        ...
        if not os.path.exists(dst):
            os.mkdir(dst)
        if not os.path.exists(dst):
            logger.error("The destination folder %s does not exist", dst)
            raise OSError("The destination folder", dst,"does not exist")
        elif not os.path.isdir(dst):
            logger.error("The destination folder %s is not a directory", dst)
