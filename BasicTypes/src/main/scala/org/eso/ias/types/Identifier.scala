...
object Identifier {
  ...
  /**
    * Get and return the number of the instance of the template from
    * a templated id.
    *
    * @param id the identifier
    * @return the number of the instance of the template;
    *         empty if the identifier does not contain a template
    */
  ...
  /**
    ...
    *
    * @return the identifier without the instance;
    *         id if does not contain a template
    */
  def getBaseId(id: String) : String = {
    require(Option(id).isDefined && id.nonEmpty)
    val instance = Identifier.getTemplateInstance(id)

    if (instance.isDefined) {
      // Templated ID
      val templatePattern = buildTemplatePattern(instance.get)
      id.replace(templatePattern,"")
    } else {
      // Non templated
      id
    }
  ...
  def buildTemplatePattern(instance: Int): String = {
    "%s%d%s".format(Identifier.templatedIdPrefix,instance,Identifier.templateSuffix)
  }
...
extends {
  ...
  /**
    ...
    */
  lazy val templateInstance: Option[Int] = Identifier.getTemplateInstance(id)

  /**
    * The Identifier without the template part if template, otherwise it the same as the [[id]]
    */
  lazy val baseId: String = Identifier.getBaseId(id)
