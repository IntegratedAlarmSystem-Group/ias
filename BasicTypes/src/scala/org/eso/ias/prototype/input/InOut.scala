 ...
 *                           It is None only and only if the value is not generated by a IASValue directly
 *                           like the output of the ASCE
...
case class InOut[A](
  ...
  def getValidity(dependantIasios: Option[Set[InOut[_]]]): Validity = {
    require( 
        if (dependantIasios.isDefined) !dependantIasios.get.isEmpty else true,
        "Invalid empty set of dependant IASIOs")
    
    // Check the last update time against the actual time 
    val timeValiditySet=Set(assessTimeValidity())
    
    val validitiesOfdepInputsSet = {
      dependantIasios match {
        case None => Set[Validity]()
        case Some(iasios) => iasios.map(i => i.getValidity(None)) 
      }
    }
    
    val iasValValiditySet = {
      fromIasValueValidity match {
        case None => Set[Validity]()
        case Some(v) => Set[Validity](v) 
      }
    }
    
    val validities = ((timeValiditySet++validitiesOfdepInputsSet++iasValValiditySet).map(_.iasValidity))
