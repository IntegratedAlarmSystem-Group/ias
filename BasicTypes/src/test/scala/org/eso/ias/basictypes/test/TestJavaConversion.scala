...
class TestJavaConversion  extends AnyFlatSpec {
  ...
  trait Fixture {
    ...
    val intHIO = new InOut[Int]( // Input
      ...
      IASTypes.INT,
      None,None,None,None,None,None,None,None,None)
    val shortHIO =  new InOut[Short]( // Input
      shortValue,
      doubleHioId,
      mode,
      validity,
  ...
  it must "correctly build the IASValue" in new Fixture {
    val doubleVal = doubleHIO.toIASValue()
    assert(doubleVal.valueType==doubleHIO.iasType)
    assert(doubleVal.mode==doubleHIO.mode)
    ...
    assert(doubleVal.id==doubleHIO.id.id)
    assert(doubleVal.fullRunningId==doubleHIO.id.fullRunningID)
    assert(doubleVal.value==doubleHIO.value.get)
    assert(doubleVal.iasValidity==IasValidity.RELIABLE)
    
    val alarmVal = alarmHIO.toIASValue()
    assert(alarmVal.value==alarmHIO.value.get)
    assert(alarmVal.iasValidity==IasValidity.RELIABLE)
  }
  
  it must "Update the times in the IASValue" in new Fixture {
    val doubleVal = doubleHIO.toIASValue()
  ...
  it must "Update a IASIO with the values from a IASValue" in new Fixture {
    ...
    val dV: IASValue[Double] = doubleHIO.toIASValue()
    ...
    assert(alarmHio.value.get.asInstanceOf[Alarm]==alarmCleared)
  ...
  it must "Update a IASIO of type TIMESTAMP with the values from a IASValue" in new Fixture {
    val tStampIasVal = tStampHIO.toIASValue()
    assert(tStampIasVal.value==tStampValue.get)
    ...
    val iasio = tStampHIO.update(newTimestampVal)
  ...
  it must "Update IASIOs of type ARRAY with the values from IASValues" in new Fixture {
    val arrayLongIasVal = arrayLongHIO.toIASValue()
    assert(arrayLongIasVal.value==arrayLongValue.get)

    val arrayLongValue2: NumericArray = new NumericArray(List(3,4,5,6,7),NumericArray.NumericArrayType.LONG)
    val newArrayLongVal = IASValue.build(arrayLongValue2, OperationalMode.OPERATIONAL, UNRELIABLE, arrayLongIasVal.fullRunningId, IASTypes.ARRAYOFLONGS)
    val iasioL = arrayLongHIO.update(newArrayLongVal)
    ...
    val arrayDoubleIasVal = arrayDoubleHIO.toIASValue()
    assert(arrayDoubleIasVal.value==arrayDoubleValue.get)

    val arrayDoubleValue2: NumericArray = new NumericArray(List(1.7,-12,3.125,-5.5,789.2345),NumericArray.NumericArrayType.DOUBLE)
    val newArrayDoubleVal = IASValue.build(arrayDoubleValue2, OperationalMode.OPERATIONAL, UNRELIABLE, arrayDoubleIasVal.fullRunningId, IASTypes.ARRAYOFDOUBLES)
    val iasioD = arrayDoubleHIO.update(newArrayDoubleVal)
  ...
  it must "Update a IASIO with the ids of dependents of a IASValue" in new Fixture {
    ...
    val alarmVal = alarmHIO2.toIASValue()
    ...
    val alarmHIO3 = new InOut[Alarm]( // Output
        alarmValue,
        alarmHioId,
        mode,
        None,
    ...
    val deps = alarmHIO3.idsOfDependants
    assert(deps.isDefined)
    assert(alarmHIO3.idsOfDependants.get.size==2)
    assert(alarmHIO3.idsOfDependants.get.contains(depId1))
    assert(alarmHIO3.idsOfDependants.get.contains(depId2))
  }
  
  it must "Update a IASIO with the properties of a IASValue" in new Fixture {
    // The IDs of dependents
    val supervId1 = new Identifier("SupervId1",IdentifierType.SUPERVISOR,None)
    val dasuId1 = new Identifier("dasuVID1",IdentifierType.DASU,supervId1)
    val asceId1 = new Identifier("asceVID1",IdentifierType.ASCE,Option(dasuId1))      
    ...
    val valuePropsOpt = alarmHIO3.props
