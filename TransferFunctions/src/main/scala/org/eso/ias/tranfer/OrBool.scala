...
/**
  * OrBool is a simple transfer function that takes as input n (n>=2) boolean IASIO values 
  * and returns the result of the OR operation between them.
  * 
  * The output produced by this TF is a boolean IASIO. The output can be inverted by setting 
  * the "org.eso.ias.tf.orbool.invert" property to true in the CDB.
  * 
  * 
  ...
  * @author acaproni
  */
class OrBool(cEleId: String, cEleRunningId: String, validityTimeFrame: Long, props: Properties) 
extends ScalaTransferExecutor[Boolean](cEleId,cEleRunningId,validityTimeFrame,props) {
   ...
   */
  val invert: Boolean = Option(props.getProperty(OrBool.InvertLogicPropName)).
    map(java.lang.Boolean.valueOf(_).booleanValue()).
    getOrElse(OrBool.DefaultLogicPropValue.booleanValue())
  ...
  override def initialize(inputsInfo: Set[IasioInfo], outputInfo: IasioInfo): Unit = {
    OrBool.logger.debug("Initializing")
    ...
    OrBool.logger.debug("Initialized")
  }
  ...
  def eval(compInputs: Map[String, IasIO[?]], actualOutput: IasIO[Boolean]): IasIO[Boolean] = {
    val orResult = compInputs.values.map(_.value.get.asInstanceOf[Boolean]).reduce(_ || _)
    return actualOutput.updateValue(if (invert) !orResult else orResult)
  }
}
...
object OrBool {
  ...
  /** The logger */
  private val logger: Logger = IASLogger.getLogger(OrBool.getClass)

  /** The name of the boolean property to invert the logic */
  val InvertLogicPropName: String = "org.eso.ias.tf.orbool.invert"
