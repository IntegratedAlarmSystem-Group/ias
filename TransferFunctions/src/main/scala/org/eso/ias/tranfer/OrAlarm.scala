...
/**
  * OrAlarm transfer function takes n alarms in input and returns another alarm 
  * that is set if at least one of the alarms in input is set (OR).
  * 
  ...
  * @author acaproni
  */
class OrAlarm (cEleId: String, cEleRunningId: String, validityTimeFrame: Long, props: Properties) 
extends ScalaTransferExecutor[Alarm](cEleId,cEleRunningId,validityTimeFrame,props) {
    ...
    */
    val invert: Boolean = Option(props.getProperty(OrAlarm.InvertLogicPropName)).
        map(java.lang.Boolean.valueOf(_).booleanValue()).
        getOrElse(OrAlarm.DefaultLogicPropValue.booleanValue())

    /**
    * The priority of the alarm from the java property or the default if not defined
    */
    val priorityFromCDB: Priority =
        Option(props.getProperty(OrAlarm.alarmPriorityPropName)).map(Priority.valueOf(_)).getOrElse(Priority.getDefaultPriority)
    ...
    **/
    override def initialize(inputsInfo: Set[IasioInfo], outputInfo: IasioInfo): Unit = {
        ...
        OrAlarm.logger.info("The OrAlarm TF accepts {} alarm inputs", inputsInfo.size)
        OrAlarm.logger.debug("Initialized")
    }
    ...
    */
    ...
    def eval(compInputs: Map[String, IasIO[?]], actualOutput: IasIO[Alarm]): IasIO[Alarm] = {
        ...
        // The alarm in output is set if there is at least one active alarm in input
        val cond: Boolean = activeAlarms.size>0
        val newAlarm: Alarm = if (!invert) actualAlarm.setIf(cond) else actualAlarm.setIf(!cond)

        // The properties of the output
        val props = if (newAlarm.isSet) {
            val idOfActiveAlarms = activeAlarms.map(_.id).mkString(",")
            getPropsOfOutput(activeAlarms.map(_.props)) + (OrAlarm.inputAlarmsSetPropName->idOfActiveAlarms)
        } else {
...
object OrAlarm {
    /** The logger */
    private val logger: Logger = IASLogger.getLogger(OrAlarm.getClass)
    ...
    val InvertLogicPropName: String = "org.eso.ias.tf.oralarm.invert"
