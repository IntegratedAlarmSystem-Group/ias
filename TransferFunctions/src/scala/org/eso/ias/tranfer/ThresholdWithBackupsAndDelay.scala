...
class ThresholdWithBackupsAndDelay(asceId: String, asceRunningId: String, validityTimeFrame:Long, props: Properties)
  ...
  override def eval(compInputs: Map[String, IasIO[_]], actualOutput: IasIO[Alarm]): IasIO[Alarm] = {
    ...
    val newOutput = {
      if (actualOutputAlarm.isEmpty || actualOutputAlarm.get==requestedAlarmByThreshold) {
        // If the output isundefined or it matches with the new request state
        ...
        if ( (requestedAlarmByThreshold==Alarm.CLEARED && delayFromLastChange>=delayToClear) ||
        (requestedAlarmByThreshold==alarmPriority && delayFromLastChange>=delayToSet)) {
          requestedAlarmByThreshold
        } else {
