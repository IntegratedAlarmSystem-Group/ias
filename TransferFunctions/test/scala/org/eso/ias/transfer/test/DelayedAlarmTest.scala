...
class DelayedAlarmTest extends FlatSpec {
  ...
  val initialOutput: InOut[Alarm] = InOut.asOutput(
    outId,
    IASTypes.ALARM)
  ...
  it must "produce a CLEARED alarm at the beginning" in {
    ...
    val tf = new DelayedAlarm(compID.id,compID.fullRunningID,props)
    tf.initialize()
    val map = Map[String, InOut[_]]( initialOutput.id.id -> initialOutput.updateValue(Some(Alarm.getSetDefault)))
  ...
  it must "clear the alarm if the input remains clear in the time interval" in {
    ...
    val mapToUnset = Map[String, InOut[_]]( initialOutput.id.id -> initialOutput.updateValue(Some(Alarm.CLEARED)))
    while (System.currentTimeMillis() < expectedTimeChange) {
      logger.info("Sending input {}",map.values.head.value.get)
  ...
  it must "be steady if the input oscillates" in {
    ...
    val mapSet = Map[String, InOut[_]]( initialOutput.id.id -> initialOutput.updateValue(Some(Alarm.getSetDefault)))
    val mapUnset = Map[String, InOut[_]]( initialOutput.id.id -> initialOutput.updateValue(Some(Alarm.CLEARED)))
