...
class DelayedAlarmTest extends FlatSpec {
  ...
  val initialOutput: InOut[AlarmSample] = InOut.asOutput(
    outId,
    IASTypes.ALARM)
  
  // Build the IASIO in input
  val initialinput= InOut.asInput(
  ...
  it must "throw en exception when inited with missing or wrong times" in {
  ...
  it must "produce a CLEARED alarm at the beginning" in {
    ...
    val tf = new DelayedAlarm(compID.id,compID.fullRunningID,props)
    tf.initialize()
    val map = Map[String, InOut[_]]( initialOutput.id.id -> initialOutput.updateValue(Some(AlarmSample.SET)))
    
    val newOutput = tf.eval(map, initialOutput)
    
    assert(newOutput.value.isDefined)
    assert(newOutput.value.get==AlarmSample.CLEARED)
  }
  ...
  it must "set the alarm if the input does not change in the time interval" in {
    ...
    while (System.currentTimeMillis() < expectedTimeChange) {
      ...
      assert(output.value.get==AlarmSample.CLEARED)
    ...
    assert(newOutput.value.get==AlarmSample.SET)
  }
  ...
  it must "clear the alarm if the input remains clear in the time interval" in {
    ...
    assert(output.value.isDefined)
    logger.info("TF produced {}",output.value.get)
    assert(output.value.get==AlarmSample.SET)
    
    val initialTime = System.currentTimeMillis()
    val expectedTimeChange = initialTime+TimeUnit.MILLISECONDS.convert(timeToClear, TimeUnit.SECONDS)
    
    val mapToUnset = Map[String, InOut[_]]( initialOutput.id.id -> initialOutput.updateValue(Some(AlarmSample.CLEARED)))
    while (System.currentTimeMillis() < expectedTimeChange) {
      logger.info("Sending input {}",map.values.head.value.get)
  ...
  it must "be steady if the input oscillates" in {
    ...
    val mapSet = Map[String, InOut[_]]( initialOutput.id.id -> initialOutput.updateValue(Some(AlarmSample.SET)))
    val mapUnset = Map[String, InOut[_]]( initialOutput.id.id -> initialOutput.updateValue(Some(AlarmSample.CLEARED)))
