 ...
 * @param id the identifier of the DASU
 * @param outputId the output generated by the DASU
 */
class DasuTopology(
  ...
  val asceTopology: Map[String,AsceTopology] = asces.foldLeft(Map.empty[String,AsceTopology]) { (z, asceTopology) =>
    z+(asceTopology.id -> asceTopology)
  }
  ...
  val inputsOfAsce: Map[String, Set[String]] = {
    asceIds.foldLeft(Map.empty[String, Set[String]]) { (z, asceId) =>
      z+(asceId -> asceTopology(asceId).inputsIds)
    }
  }
  ...
  val ascesOfInput: Map[String, Set[String]] = {
    val allInputs: Set[String] = asces.flatMap(asce => asce.inputsIds).toSet

    def ascesWithInput(x: String) = {
      asces.foldLeft(Set.empty[String]) { (z, asce) =>
        if (asce.inputsIds.contains(x)) z+asce.id
        else z
      }
    }
  ...
  linearizedTrees.foreach(tree => require(checkLastNodeId(tree,outputId)))
  ...
  private def buildTreeOfNode(node: Node): Node = {
    ...
    val childs: List[Node] = connections.map(asce => buildTreeOfNode(new Node(asce.outputId,NodeType.ASCE,List())))
  ...
  private def buildTrees(): Set[Node] = {
    inputsIds.map(inId => buildTreeOfNode(new Node(inId, NodeType.IASIO, List())))
  }
  ...
  def buildLevels(): List[Set[String]] = {
    ...
    def allocate(availableInputs: Set[String], allocatedAsces: Set[String], acc: List[Set[String]]):List[Set[String]] = {
      ...
      val s: List[AsceTopology] = asces.filter(asce => asce.inputsIds.forall(input => availableInputs.contains(input)))
      // The identifiers of the ASCEs belonging to this level
      val asceIds = s.map(asce => asce.id).toSet--allocatedAsces
      // The new available inputs enriched with the outputs
      // generated by the ASCEs at this level
      val newInputs = availableInputs++s.map(asce => asce.outputId)

      val newAllocatedAsces = allocatedAsces++asceIds
      val ascesToAllocate = asces.map(_.id).filterNot(id => newAllocatedAsces.contains(id)).toSet
    ...
    allocate(inputsIds,Set[String](),Nil)
  }
